...
class FiberInstallationVerifier:
    ...
    def verify_step(self, image_path: str, step_number: int) -> VerificationResult:
        ...
        Args:
            ...
            step_number: Installation step number (1-12)
        ...
        step_keys = {
            1: "step1_frontage",
            2: "step2_cable_span",
            3: "step3_entry_outside_closeup",
            4: "step4_entry_inside",
            5: "step5_wall_before",
            6: "step6_fiber_to_ont",
            7: "step7_powermeter_ont",
            8: "step8_ont_barcode",
            9: "step9_mini_ups",
            10: "step10_work_area_complete",
            11: "step11_active_light",
            12: "step12_customer_signature"
        }
        ...
        if step_key not in self.prompts:
            return VerificationResult(
                ...
                confidence=1.0,
                recommendation="Please use a valid step number (1-12)"
            )
        ...
        try:
            ...
            try:
                ...
                # Apply strictness threshold - this overrides the AI's raw "passed" decision
                # The final pass/fail is based on score vs. our configured threshold
    ...
    def _parse_text_response(self, response_text: str, step_number: int) -> VerificationResult:
        ...
        ai_suggests_pass = any(keyword in text_lower for keyword in passed_keywords)
        ai_suggests_fail = any(keyword in text_lower for keyword in failed_keywords)

        # Default to failed if uncertain
        ai_suggests_pass = ai_suggests_pass and not ai_suggests_fail
        
        # Estimate score based on AI suggestion
        estimated_score = 8 if ai_suggests_pass else 4
        
        # Apply strictness threshold like we do for JSON responses
        passed = estimated_score >= Config.PASSING_SCORE_THRESHOLD
    ...
    def verify_installation_batch(self, photos: Dict[int, str]) -> Dict:
        ...
        completion_rate = len(photos) / 12 * 100  # Updated to 12 steps
        ...
        status = "PASS" if (
            ...
            passed_steps >= 10  # At least 10 out of 12 steps must pass
        ) else "FAIL"

        return {
            "overall_score": round(average_score, 1),
            "completion_rate": round(completion_rate, 1),
            "total_steps": 12,  # Updated to 12 steps
